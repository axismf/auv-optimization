...
_DARK_RC: dict = {
    "figure.facecolor":  "#040C18",
    "axes.facecolor":    "#0A1830",
    "axes.edgecolor":    "#1E3A5C",
    "axes.labelcolor":   "#C5D5E8",
    "xtick.color":       "#C5D5E8",
    "ytick.color":       "#C5D5E8",
    "text.color":        "#C5D5E8",
    "grid.color":        "#1A3050",
    "axes.titlecolor":   "#C5D5E8",
    "legend.facecolor":  "#060F1E",
    "legend.edgecolor":  "#1E3A5C",
    "legend.labelcolor": "#C5D5E8",
}
plt.rcParams.update(_DARK_RC)

_PLOTLY_DARK = dict(
    template="plotly_dark",
    paper_bgcolor="#040C18",
    font=dict(color="#C5D5E8"),
)
...
def plot_3d(
    ...
    fig.update_layout(
        **_PLOTLY_DARK,
        scene=dict(
            bgcolor="#040C18",
            xaxis=dict(backgroundcolor="#0A1830", gridcolor="#1A3050", color="#C5D5E8",
                       title="Longitud [°]"),
            yaxis=dict(backgroundcolor="#0A1830", gridcolor="#1A3050", color="#C5D5E8",
                       title="Latitud [°]"),
            zaxis=dict(
                backgroundcolor="#0A1830", gridcolor="#1A3050", color="#C5D5E8",
                title="Profundidad",
                autorange="reversed",
        ...
        legend=dict(x=0.01, y=0.99, bgcolor="#060F1E", bordercolor="#1E3A5C"),
...
def plot_bateria(
    ...
    niveles_wh = [n / 3600 for n in niveles]
    e_max_wh = e_max / 3600

    for k in range(len(niveles_wh) - 1):
        if niveles_wh[k + 1] > niveles_wh[k]:
            ax.axvspan(dist_km[k], dist_km[k + 1], alpha=0.25, color="green")
        elif niveles_wh[k + 1] < niveles_wh[k]:
            ax.axvspan(dist_km[k], dist_km[k + 1], alpha=0.12, color="orange")

    umbral = 0.20 * e_max_wh
    ax.axhspan(0, umbral, alpha=0.15, color="red", label="Zona crítica (<20%)")
    ax.axhline(e_max_wh, color="gray", linewidth=1.0, linestyle="--",
               label=f"Capacidad máx. ({e_max_wh:.0f} Wh)")
    ax.plot(dist_km, niveles_wh, color="#1f77b4", linewidth=2.0, label="Batería [Wh]")
    ax.fill_between(dist_km, niveles_wh, alpha=0.15, color="#1f77b4")
...
def plot_tours_atsp(
    ...
    for k in range(len(orden_optimo) - 1):
        ...
        ax_ciclo.text(
            mx, my, etiq_cost,
            fontsize=7, ha="center", va="center", color="#C5D5E8",
            bbox=dict(boxstyle="round,pad=0.15", fc="#0D1F33", ec="none", alpha=0.85),
        )
...
def plot_comparativa_algoritmos(
    costos: dict[str, float],
    ax: plt.Axes | None = None,
) -> plt.Axes:
    """Compara el consumo energético total de la ruta según el algoritmo usado (RF-07).

    Bellman-Ford es el único capaz de aprovechar aristas de costo negativo
    (regeneración de batería por corrientes favorables). Dijkstra y A* exigen
    pesos no negativos para garantizar corrección, por lo que ambos deben
    ignorar esa regeneración y llegan al mismo consumo total: A* solo mejora
    la velocidad de búsqueda con su heurística, no encuentra un costo distinto.

    Args:
        costos: Dict {nombre_algoritmo: costo_total_J}. Se asume que el valor
                más bajo corresponde a la ruta realmente óptima (Bellman-Ford).
        ax:     Ejes opcionales donde dibujar.
    """
    if ax is None:
    ...
    valores = [v / 3_600.0 for v in costos.values()]
    minimo  = min(valores)
    colores = ["#2ca02c" if abs(v - minimo) < 1e-9 else "#d62728" for v in valores]

    barras = ax.bar(nombres, valores, color=colores, width=0.55, zorder=2)
    ...
    if len(valores) > 1 and max(valores) > 0:
        ahorro_pct = (max(valores) - minimo) / max(valores) * 100
        if ahorro_pct > 0.05:
            ax.annotate(
                f"−{ahorro_pct:.1f}% vs. sin regeneración",
                xy=(0, minimo), xytext=(0, 14), textcoords="offset points",
                ha="center", fontsize=8, color="#2ca02c", fontweight="bold",
            )

    ax.set_ylabel("Costo energético total [Wh]")
    ax.set_title("Consumo de la ruta según algoritmo\n(BF aprovecha regeneración; Dijkstra/A* no pueden)")
    ax.grid(axis="y", alpha=0.3, zorder=1)
    ax.set_ylim(0, max(valores) * 1.3 if valores else 1.0)
    return ax
...
def plot_grafo_3d(
    grafo: Grafo,
    campo: CampoCorrientes,
    max_aristas: int = 6_000,
    *,
    ruta: list[Nodo] | None = None,
    waypoints: list[Nodo] | None = None,
    centinelas: list[Nodo] | None = None,
    base: Nodo | None = None,
) -> go.Figure:
    """Visualización 3D interactiva del grafo dirigido generado (RF-03, RF-07).

    Cada arista se colorea según el signo de su costo energético:
    verde = costo negativo (el AUV regenera batería con la corriente),
    rojo  = costo positivo (el AUV consume batería para propulsarse).

    Cuando se pasa `ruta`, filtra el grafo a los tramos recorridos y añade
    la línea de ruta con marcadores de zonas y base (vista de misión final).

    Args:
        grafo:       Grafo dirigido y ponderado del dominio (nodos navegables).
        campo:       Campo de corrientes (para convertir índices a coordenadas).
        max_aristas: Límite de aristas al mostrar el grafo completo (muestra aleatoria).
        ruta:        Si se provee, filtra a los tramos recorridos y añade marcadores.
        waypoints:   Zonas de convergencia (solo cuando ruta is not None).
        centinelas:  Zonas centinela (solo cuando ruta is not None).
        base:        Nodo base de la misión (solo cuando ruta is not None).

    Returns:
        Figura Plotly con los nodos y las aristas del grafo.
    """
    def coords(nodo: Nodo) -> tuple[float, float, float]:
        p, i, j = nodo
        return float(campo.lon[j]), float(campo.lat[i]), float(p)

    if ruta is not None:
        ruta_edges = set(zip(ruta, ruta[1:]))
        aristas = [(u, v, w) for u, v, w in grafo.aristas()
                   if math.isfinite(w) and (u, v) in ruta_edges]
        muestreado = False
    else:
        aristas = [(u, v, w) for u, v, w in grafo.aristas() if math.isfinite(w)]
        muestreado = len(aristas) > max_aristas
        if muestreado:
            rng = np.random.default_rng(42)
            idx = rng.choice(len(aristas), size=max_aristas, replace=False)
            aristas = [aristas[k] for k in idx]

    xs_pos: list = []; ys_pos: list = []; zs_pos: list = []
    xs_neg: list = []; ys_neg: list = []; zs_neg: list = []
    n_pos = n_neg = 0
    ...
    if xs_pos:
        traces.append(go.Scatter3d(
            x=xs_pos, y=ys_pos, z=zs_pos,
            mode="lines",
            line=dict(color="#d62728", width=2),
            opacity=0.45,
            name=f"Costo + (consumo) · {n_pos}",
            hoverinfo="skip",
        ))
    if xs_neg:
        traces.append(go.Scatter3d(
            x=xs_neg, y=ys_neg, z=zs_neg,
            mode="lines",
            line=dict(color="#2ca02c", width=3),
            opacity=0.85,
            name=f"Costo − (regeneración) · {n_neg}",
            hoverinfo="skip",
        ))

    if ruta is not None:
        ruta_nodos = list(dict.fromkeys(ruta))
        traces.append(go.Scatter3d(
            x=[float(campo.lon[j]) for _, _, j in ruta_nodos],
            y=[float(campo.lat[i]) for _, i, _ in ruta_nodos],
            z=[float(p)            for p, _, _ in ruta_nodos],
            mode="markers",
            marker=dict(color="#5A5A5A", size=3, symbol="circle"),
            name=f"Nodos ruta · {len(ruta_nodos)}",
            hovertemplate="Lon: %{x:.4f}°<br>Lat: %{y:.4f}°<extra></extra>",
        ))

        lons   = [float(campo.lon[j]) for _, _, j in ruta]
        lats   = [float(campo.lat[i]) for _, i, _ in ruta]
        z_ruta = [float(p)            for p, _, _ in ruta]
        traces.append(go.Scatter3d(
            x=lons, y=lats, z=z_ruta,
            mode="lines",
        ...
        if waypoints:
            traces.append(go.Scatter3d(
                x=[float(campo.lon[j]) for _, _, j in waypoints],
                y=[float(campo.lat[i]) for _, i, _ in waypoints],
                z=[float(p)            for p, _, _ in waypoints],
                mode="markers+text",
                marker=dict(color="red", size=8, symbol="diamond"),
                text=[f"C{k+1}" for k in range(len(waypoints))],
        ...
        if centinelas:
            traces.append(go.Scatter3d(
                x=[float(campo.lon[j]) for _, _, j in centinelas],
                y=[float(campo.lat[i]) for _, i, _ in centinelas],
                z=[float(p)            for p, _, _ in centinelas],
                mode="markers+text",
                marker=dict(color="cyan", size=8, symbol="diamond"),
                text=[f"S{k+1}" for k in range(len(centinelas))],
        ...
        if base is not None:
            pb, ib, jb = base
            traces.append(go.Scatter3d(
                x=[float(campo.lon[jb])], y=[float(campo.lat[ib])], z=[float(pb)],
                mode="markers+text",
                marker=dict(color="lime", size=10, symbol="square"),
                text=["Base"], textposition="top center",
                name="Base",
        ...
        n_tramos = len(ruta_edges)
        titulo = f"Ruta del AUV — {len(ruta_nodos)} nodos · {n_tramos} tramos"
    else:
        nodos = list(grafo.nodos())
        traces.append(go.Scatter3d(
            x=[float(campo.lon[j]) for _, _, j in nodos],
            y=[float(campo.lat[i]) for _, i, _ in nodos],
            z=[float(p)            for p, _, _ in nodos],
            mode="markers",
            marker=dict(color="#5A5A5A", size=3, symbol="circle"),
            name=f"Nodos · {len(nodos)}",
            hovertemplate="Lon: %{x:.4f}°<br>Lat: %{y:.4f}°<extra></extra>",
        ))
        subtitulo = " (muestra parcial de aristas)" if muestreado else ""
        titulo = f"Grafo dirigido — {grafo.num_nodos} nodos · {grafo.num_aristas} aristas{subtitulo}"
