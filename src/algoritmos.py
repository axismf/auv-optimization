...
def matriz_costos_sin_corrientes(
    grafo_con: Grafo,
    waypoints: list[Nodo],
    campo: CampoCorrientes,
    params: ParametrosModelo,
) -> np.ndarray:
    """Construye matriz de costos IGNORANDO beneficios de corriente (solo arrastre + gravedad).

    Simula un enfoque "Dijkstra-like" donde no se aprovecha la regeneración:
    - Calcula la distancia geométrica entre pares de waypoints
    - Aplica solo el costo de arrastre (e_drag) + gravedad (e_grav)
    - NO considera regeneración por corrientes

    Args:
        grafo_con:  Grafo original (para conocer los nodos y distancias).
        waypoints:  Lista de waypoints.
        campo:      Campo de corrientes (para coordenadas).
        params:     Parámetros del modelo.

    Returns:
        M_sin[k, k]: matriz de costos sin beneficio de corrientes.
    """
    import math
    from .grafo import Nodo, costo_arista

    k = len(waypoints)
    M_sin = np.full((k, k), math.inf)
    ...
    for i, origen in enumerate(waypoints):
        ...
        for j, destino in enumerate(waypoints):
            ...
            # Velocidad relativa al agua SIN corriente (solo propulsión)
            vr3 = (params.s ** 2) ** 1.5  # ‖s·ê‖³ = s³

            # Tiempo de tránsito (sin corriente: v_g = s)
            tiempo = L / params.s
            ...
            # Energía de arrastre (sin corriente)
            e_drag = params.k_p * vr3 * tiempo

            # Sin regeneración: costo total = arrastre + gravedad
            M_sin[i, j] = e_drag + e_grav
